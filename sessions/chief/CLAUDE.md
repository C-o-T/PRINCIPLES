...
**경고를 수신한 순간, 사용자는 이미 알고 있다.**
감시 세션은 chief에게 전달하는 동시에 사용자에게 직접 보고한다.
chief의 해명을 감시 세션이 수용·거부하지 않는다 — 사용자가 최종 판단한다.

```
수용 → RCA 근거 + 수정 계획을 사용자에게 직접 보고 + ACTIVE_CONTEXT.md 업데이트
반박 → rca 세션 위임 필수 → rca 결과를 사용자에게 직접 제출 (구두 주장만으로 반박 불가)
무시 → sentinel이 "chief 미응답" 추가 보고 → VIOLATION_LOG 가중 기록
```

**아래 응답은 수용·반박으로 인정되지 않는다 (변명 패턴 — 사용 시 위반 확정):**
- "환경 제약이라서" / "다음엔 안 할게" / "예외 상황이었다" / "결과가 좋았다" / "빠르게 처리하려고"
